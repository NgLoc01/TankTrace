...
#Builds and sets the correct paths for input/output video
VIDEOS_DIR = os.path.join('.', 'videos')
video_path = os.path.join(VIDEOS_DIR, 'tank1.mp4')
video_path_out = '{}_untrained_out.mp4'.format(video_path)
video_path_silent = '{}_untrained_out.silent.mp4'.format(video_path)

#Open the video file and grab its first frame
cap = cv2.VideoCapture(video_path)  #opens the video as a stream and returns a capture object you can read frames from
...
#Load the base model (no tank training) and set up detection settings, for a before/after baseline vs predict_trained.py
model = YOLO("yolov8s.pt")  #base COCO weights, no sherman_tank class
...
progress = tqdm(total=total_frames, desc="Detecting (untrained)", unit="frame")
...
while ret:
    ...
    for result in results.boxes.data.tolist():  #loop over every object detected in this frame (can be several tanks in the frame)
        ...
        if score > threshold:  #only draw detections the model is confident enough about
            predicted_class = model.names[int(class_id)]  #actual COCO label the model predicted, e.g. "truck", "car"
            cv2.rectangle(frame, (int(x1), int(y1)), (int(x2), int(y2)), (0, 0, 255), 4)  #draw the box
            cv2.putText(frame, f"{predicted_class.upper()} ({score:.2f})", (int(x1), int(y1 - 10)),
                        cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 3, cv2.LINE_AA)  #label the box
